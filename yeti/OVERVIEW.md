...
- **Ownership via `GeneratedMarker`** (added after PR #137 review flagged
  that name-in-component was too weak an ownership signal): every
  generated file starts with `# Generated by updex catalog (repo: <name>)
  ...`. `CatalogAdd` errors instead of overwriting existing files without
  the marker; `CatalogRemove` only recognizes a sysext as catalog-managed
  when `<etc component dir>/<name>.feature` carries the marker
  (`catalog.IsGeneratedFile`), and skips deleting a non-generated
  `.transfer`. This is what makes a `Component=docker` override safe
  against an existing hand-managed docker component.
- **Name validation**: `catalog.ValidateSysextName`
  (`^[a-zA-Z0-9_][a-zA-Z0-9._+-]*$`) runs at the top of
  `CatalogAdd`/`CatalogRemove` and inside `FetchConf`, so traversal-shaped
  names (`../x`, dotfiles) never reach `filepath.Join` or URL
  construction.
- **Rollback**: when a *fresh* `CatalogAdd` fails during
  enable/download, the generated `.transfer`/`.feature`, the
  `.feature.d` drop-in dir, and the component dir (if empty) are removed
  so no enabled-but-broken state persists. A re-add of an
  already-catalog-owned sysext leaves the previous files in place on
  failure.
