...
Generated files carry a `# Generated by updex catalog ...` header as an
ownership marker: `catalog add` refuses to overwrite definitions it did
not generate, and `catalog remove` only removes marker-owned files — so
hand-written or package-shipped definitions sharing a name or component
are never touched. A failed fresh `add` rolls its generated files back
rather than leaving a half-enabled sysext behind.
