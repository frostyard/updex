...
| Coverage | [Tests workflow runs](https://github.com/frostyard/updex/actions/workflows/test.yml) — Unit Tests job, step `Enforce coverage floor` (`make coverage-check`, [`scripts/check-coverage.sh`](../../scripts/check-coverage.sh), [`.coverage-baseline`](../../.coverage-baseline)) | The enforced gate is `max(80.0, baseline - 0.5)` total statement coverage over the non-E2E packages: the 80.0% total statement-coverage floor is absolute, while a drop of more than 0.5 percentage points from the committed baseline fails the run. When a PR raises coverage, regenerate the non-E2E atomic profile, read its `go tool cover -func=coverage.out` total, and bump `.coverage-baseline` in that PR. The [Codecov dashboard](https://codecov.io/gh/frostyard/updex) is best-effort only, pending onboarding on codecov.io — the upload is `continue-on-error` and [`codecov.yml`](../../codecov.yml) records the *intended* project/patch statuses, which take effect only once the repository is onboarded; until then Codecov reports nothing and is not a gate. |
