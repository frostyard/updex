...
`DisableFeature` with `Now: true` removes only the transfers the disabled feature *released* — members that no remaining enabled feature still activates (`config.TransfersReleasedByDisabling`, the `FilterTransfersByFeatures` OR/AND predicate against the post-disable state); a transfer shared with another enabled feature keeps its image and sysext link, is absent from `RemovedFiles`, and is named in `NextActionMessage` (`Kept <component>: still activated by another enabled feature.`).
