...
| Build gate green | `make fmt` leaves no diff and `make ci` passes: tidy, `go vet`, gofmt, golangci-lint (`.golangci.yml`), non-E2E unit tests, the `max(80.0, baseline - 0.5)` total statement-coverage gate (`make coverage-check`, `scripts/check-coverage.sh`, and `.coverage-baseline`), the hermetic E2E suite, non-E2E race tests, linux amd64/arm64 builds — the same fail-fast order as [`.github/workflows/test.yml`](../../.github/workflows/test.yml). |
| Tests | New or changed behavior has focused, table-driven tests including meaningful failure paths, or the PR explains why tests do not apply; CLI/e2e changes also pass `go test -v ./cmd/updex ./tests/e2e/...`; total statement coverage stays at or above `max(80.0, baseline - 0.5)` (`make coverage-check`). A PR that raises coverage regenerates the non-E2E atomic profile and bumps `.coverage-baseline` to the `go tool cover -func=coverage.out` total in that same PR. Codecov's project (−1% max) and patch (70%) statuses from `codecov.yml` apply only once the repository is onboarded on codecov.io and are advisory until then. |
