...
| Build gate green | `make fmt` leaves no diff and `make ci` passes: tidy, `go vet`, gofmt, golangci-lint (`.golangci.yml`), non-E2E unit and race tests, linux amd64/arm64 builds — the same fail-fast order as [`.github/workflows/test.yml`](../../.github/workflows/test.yml). |
| Tests | New or changed behavior has focused, table-driven tests including meaningful failure paths, or the PR explains why tests do not apply; CLI/e2e changes also pass `go test -v ./cmd/updex ./tests/e2e/...`; Codecov's project (−1% max) and patch (70%) gates from `codecov.yml` hold. |
