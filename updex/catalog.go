...
func (c *Client) CatalogAdd(ctx context.Context, name string, opts CatalogAddOptions) (*CatalogAddResult, error) {
	...
	// Refuse to overwrite definitions that catalog add did not generate
	// (no GeneratedMarker header): a configurable Component may point at a
	// directory holding hand-written or package-shipped files, and
	// clobbering those would be irreversible configuration loss.
	existedBefore := false
	for _, path := range []string{result.TransferFile, result.FeatureFile} {
		if _, err := os.Stat(path); err != nil {
			continue
		}
		if !catalog.IsGeneratedFile(path) {
	...
	if err != nil {
		// A failed fresh add must not leave a persistent half-installed
		// state (definitions present + feature enabled would make every
		// later update retry the failed download). Re-adds keep their
		// previous, previously working files instead.
		if !existedBefore {
			c.warn("add failed; rolling back generated files")
			_ = os.Remove(result.TransferFile)
			_ = os.Remove(result.FeatureFile)
			_ = os.RemoveAll(filepath.Join(dir, name+".feature.d"))
			_ = os.Remove(dir) // only removes the directory when empty
		}
		return result, err
	}
...
func (c *Client) CatalogRemove(ctx context.Context, name string, opts CatalogRemoveOptions) (*CatalogRemoveResult, error) {
	...
	// /etc component directory carries the GeneratedMarker header written
	// by catalog add. A same-named feature that merely lives in the same
	// component (hand-written, package-shipped, or defined in a lower
	// root) must never be disabled or deleted by catalog remove.
	var owners []catalog.Repo
	for _, repo := range repos {
		featureFile := filepath.Join(config.EtcComponentDir(repo.Component), name+".feature")
		if catalog.IsGeneratedFile(featureFile) {
	...
	for _, path := range paths {
		...
		if path == transferFile && !catalog.IsGeneratedFile(path) {
			c.warn("keeping %s: not generated by updex catalog", path)
			continue
		}
		if opts.DryRun {
