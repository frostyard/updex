...
func (c *Client) CatalogAdd(ctx context.Context, name string, opts CatalogAddOptions) (*CatalogAddResult, error) {
	...
	// re-add rolls back to its previously working definitions.
...
func snapshotFile(path string) fileSnapshot {
	...
	s.data, s.mode, s.existed = data, info.Mode().Perm(), true
	return s
}

// restore rewrites the captured contents, or removes the file when it did
// not exist at snapshot time. Best-effort: the caller is already returning
...
func (c *Client) CatalogRemove(ctx context.Context, name string, opts CatalogRemoveOptions) (*CatalogRemoveResult, error) {
	...
	for _, path := range paths {
		...
		if path == transferFile {
			if owner, ok := catalog.GeneratedFileRepo(path); !ok || owner != repo.Name {
				c.warn("keeping %s: not generated by catalog %q", path, repo.Name)
				continue
			}
		}
		if opts.DryRun {
